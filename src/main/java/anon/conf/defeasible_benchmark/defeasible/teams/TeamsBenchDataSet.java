package anon.conf.defeasible_benchmark.defeasible.teams;
...
public class TeamsBenchDataSet extends DefeasibleBenchDataSet {
	private static final String NAME = "TeamsBench";
	
	private int[] sizes;
	private Iterable<? extends Object> KBGeneratorIterable;
	
	public TeamsBenchDataSet(int[] sizes) {
		this.sizes = sizes;
	...
	class TeamsKBStructureGenerator extends AbstractKBStructureGenerator {
		...
		private void generateForAtom(KBStructure kb, String num) {
			...
			for(int k=0; k < 4; k++) {
				if(k == 2 ) intendedAtom = negatedAtom; 

				// Add rules
				if(k == 2) defeasible = "DEFT";
				else defeasible = "";
