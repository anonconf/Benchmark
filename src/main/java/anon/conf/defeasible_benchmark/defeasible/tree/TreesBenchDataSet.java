package anon.conf.defeasible_benchmark.defeasible.tree;
...
public class TreesBenchDataSet extends DefeasibleBenchDataSet {
	private static final String NAME = "TreesBench";
	
	private int[] sizes;
	private Iterable<? extends Object> KBGeneratorIterable;
	
	public TreesBenchDataSet(int[] sizes, int k) {
		this.sizes = sizes;
	...
	class TeamsKBStructureGenerator extends AbstractKBStructureGenerator {
		...
		private void generateForAtom(KBStructure kb, String num) {
			...
			for(int j=0; j < k; j++) {
				//if(j == 2) intendedAtom = negatedAtom; 

				// Add rules
				//if(j == 2) defeasible = "DEFT";
				//else defeasible = "";
